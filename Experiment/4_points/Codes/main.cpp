...
int main() {
    ...
    int image_numbers = 2; //how many images we use
    int point_numbers = 4; //the number of total points,E.X. if it is set to 4, it means there are two rows and
    //two points on each row.
    int board_w = static_cast<int>(sqrt(point_numbers));
    float magnitude_noise = 1.5;
